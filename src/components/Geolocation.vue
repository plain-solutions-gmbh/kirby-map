<template>
     <k-field class="k-geolocation-field" :label="label" :required="required" :disabled="disabled">
        ...
        <k-dropdown>
            ...
            <k-dropdown-content ref="dropdown">
                <k-dropdown-item v-for="(option, index) in dropdownOptions"
                    ...
                    <span v-html="$t('maps.field.geolocation.'+option.type) + ': ' + option.name" />
                </k-dropdown-item>
            </k-dropdown-content>
    ...
    methods: {
        searchLocation(e) {
            ...
            if (e.length > 0) {
                ...
                fetch('https://api.mapbox.com/geocoding/v5/mapbox.places/' + e + '.json?types=address,country,postcode,place,locality&limit=5&access_token=' + this.token)
